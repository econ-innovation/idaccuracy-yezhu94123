...
## 题目2
## 从google专利⽂件中拆分出专利的filling date|publication date|grant date|priority date，
## 并将拆分过程写成针对⽂件的函数，从命令⾏直接运⾏，参数为⽂件名（google100.txt）
import json
with open("google100.txt", "r") as f, open('ipc_拆分作业.txt', 'a') as fs:
    for pt in f.readlines():
        patent = json.loads(pt)
        if len(patent["ipc"]) > 0:
            for ipc in patent["ipc"]:
                fs.write("{0}|{1}|{2}|{3}|{4}|{5}\n".format(
                    ipc["code"],
